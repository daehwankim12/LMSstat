...
PCA <- function(Data,
  ...
  PCA_Data_input[, 1] <-
    as.factor(PCA_Data_input[, 1]) # Group as factors
  Bef_merge <-
    prcomp(PCA_Data_input[, 2:ncol(PCA_Data_input)], scale = T)
  ...
  sem_res <-
    eval(parse(
      text = paste0(
        "ggplot2::ggplot(data = PCA_res, ggplot2::aes(x = PC",
        components[1],
        ", y =PC",
        components[2],
        ",
    ...
    ggplot2::labs(
      x = paste0(
        ...
        components[1],
        ": ",
        round(explained_variance[components[1]] * 100, 1),
        "%"
      ),
      y = paste0(
        "PC",
        components[2],
        ": ",
        round(explained_variance[components[2]] * 100, 1),
        "%"
      )
    ) +
    ggplot2::theme(
  ...
  if (ellipse == T) {
    sem_res <-
      sem_res + eval(parse(
        text = paste0(
          "ggplot2::stat_ellipse(ggplot2::aes(x = PC",
          components[1],
          ", y =PC",
          components[2],
          ",
  ...
  ggplot2::ggsave(
    paste(components[1], components[2], "label", names, "PCA.png", sep = "_"),
