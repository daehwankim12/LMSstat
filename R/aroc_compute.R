...
aroc_compute <-
  function(object,
    ...
    if (class(object)[1] != "AROC.sp") {
      stop(paste0(
        "This function can not be used for this object class: ",
        class(object)[1]
      ))
    }
    ...
    if (!missing(newdata) &&
      length(names.cov) != 0 && sum(is.na(match(
      names.cov,
      names(newdata)
    )))) {
    ...
    data.d <-
      (object$data[object$data[, object$group] != object$tag.h, ])[!object$missing.ind$d, ]
    n1 <- nrow(data.d)
    pre.placement.values <-
      (data.d[, object$marker] - predict(object$fit,
        newdata = data.d
      )) / sigma0
