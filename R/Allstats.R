...
Allstats <-
  function(Data,
    ...
    colnames(Data_renamed) <-
      c(colnames(Data[1:2]), LETTERS210729[1:nmet])
    ...
    if (length(unique(Data_final$Group)) == 2) {
      suppressWarnings({
        for (Gnum in 1:length(unique(Data$Group))) {
          assign(
            paste("Data", unique(Data$Group)[Gnum],
              sep = "_"
            ),
            dplyr::filter(
              Data_final,
          ...
          for (Gnum_2 in 1:length(unique(Data$Group))) {
            if (Gnum_2 <= Gnum_1) {
              ...
              for (met in 1:(ncol(Data) - 2)) {
                ...
                assign(
                  paste(
                    as.character(LETTERS210729[met]),
                    unique(Data$Group)[Gnum_1],
                    unique(Data$Group)[Gnum_2],
                    "T_test",
                    sep = "_"
      ...
      suppressWarnings({
        ...
        if (is.na(Result[1, Rounder]) == T) {
          for (met_1 in 1:(ncol(Data_final) - 2)) {
            Result[met_1, Rounder] <-
              eval(parse(text = paste(LETTERS210729[met_1],
                "Anova",
                sep = "_"
              )))
          }
        ...
        for (met in 1:(ncol(Data) - 2)) {
          assign(
            paste(LETTERS210729[met], "Anova_Post_Hoc",
              sep = "_"
            ),
            DescTools::PostHocTest(aov(eval(
              parse(text = LETTERS210729[met])
            ) ~
              Group, data = Data_final), method = "scheffe")
          )
        }
        for (Ano_numb in 1:nrow(eval(parse(
          text = paste(LETTERS210729[met],
            "Anova_Post_Hoc",
            sep = "_"
          )
        ))[["Group"]])) {
        ...
        for (met in 1:(ncol(Data) - 2)) {
          assign(
            paste(LETTERS210729[met], "Dunn_Post_Hoc",
              sep = "_"
            ),
            FSA::dunnTest(
              eval(parse(text = LETTERS210729[met])) ~
        ...
        for (Kru_numb in 1:length(eval(parse(
          text = paste(LETTERS210729[met],
            "Dunn_Post_Hoc",
            sep = "_"
          )
        ...
        for (nrow in 1:nrow(eval(parse(
          ...
          AN_Post_names <-
            rbind(AN_Post_names, rownames(eval(parse(
              text = paste(LETTERS210729[met],
                "Anova_Post_Hoc",
                sep = "_"
              )
        ...
        DU_post_names <- NULL
        for (nrow in 1:length(eval(parse(
          text = paste(LETTERS210729[met],
            "Dunn_Post_Hoc",
            sep = "_"
          )
          ...
          DU_post_names <-
            rbind(DU_post_names, (eval(parse(
              text = paste(LETTERS210729[met],
                "Dunn_Post_Hoc",
                sep = "_"
              )
      ...
      Result[, (2 * choose(length(unique(Data$Group)), 2) + 2):(3 * choose(length(unique(Data$Group)), 2) + 1)] <-
        Result_Ano_P
      Result[, (3 * choose(length(unique(Data$Group)), 2) + 3):(4 * choose(length(unique(Data$Group)), 2) + 2)] <-
        Result_Kru_P
      ...
      Result_Ano <- as.data.frame(Result[, (2 * choose(
        length(unique(Data$Group)),
        2
      ) + 1):(2 * choose(length(unique(Data$Group)), 2) +
        1)])
