...
#' @param color colors used for ggplots.color
...
PCoA <- function(Data,
  ...
  PCoA_Data_input <-
    Data # subset samples of interest, columns: Group(Male, Female etc)+metabolite, rownames : sample names
  PCoA_Data_input[, 1] <-
    as.factor(PCoA_Data_input[, 1]) # Group as factors
  for (method in methods) {
    Dist_Met <-
      as.matrix(vegan::vegdist(PCoA_Data_input[, 2:ncol(PCoA_Data_input)], method = method))
    Res <- ape::pcoa(Dist_Met)
    PCoA_Res <- as.data.frame(Res[["vectors"]])
    x_y_coord_G <-
      cbind(PCoA_Res[, c(components[1], components[2])], PCoA_Data_input$Group)
    colnames(x_y_coord_G)[3] <- "Group"
    M_p_value <- vegan::adonis2(Dist_Met ~ Group,
      method = method,
      by = NULL,
      data = x_y_coord_G
    )[["Pr(>F)"]][1] # Model P_Value
    sem_res <-
      ggplot2::ggplot(
        data = x_y_coord_G,
        ggplot2::aes(
      ...
      ggplot2::labs(
        x = paste0("Component ", components[1], ": ", round(Res[["values"]][["Relative_eig"]][components[1]] * 100, 1), "%"),
    ...
    if (ellipse == T) {
      sem_res <- sem_res + ggplot2::stat_ellipse(
        ggplot2::aes(
          x = Axis.1,
          y = Axis.2,
          color = Group
