#' Create and save violinplots for all metabolites with significance indicators
#'
#' This function automatically generates and saves violinplots for all metabolites in the dataset,
#' with asterisks indicating statistical significance between groups based on the selected
#' statistical test. Plots are saved as PNG files in a 'violinplot' directory.
#'
...
#' @param colour Vector of color codes used for different groups in the violinplots
...
#' @param width Width of the violinplots
#' @param size Line size for the violinplot outlines
...
#' @return Invisibly returns NULL, with plots saved to the 'violinplot' directory
#' @export
#'
#' @examples
#' \dontrun{
...
Violinplot_new <- function(data,
  ...
  dir.create("violinplot", showWarnings = FALSE)
  ...
  draw <- function(i) {
    ...
    p <- ggplot2::ggplot(dat, ggplot2::aes(Group, .data[[safe[i]]], colour = Group)) +
      ...
      ggplot2::geom_jitter(width = .1, size = 1) +
    ...
    suppressMessages(
      ggplot2::ggsave(
        file.path("violinplot", sprintf("%s_violin.png", sanitize(map[[safe[i]]]))),
        plot = p,
        device = ragg::agg_png,
        width = fig_width,
        height = fig_height,
