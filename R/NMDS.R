...
NMDS <- function(Data,
  ...
  NMDS_Data_input[, 1] <-
    as.factor(NMDS_Data_input[, 1]) # Group as factors
  for (method in methods) {
    Dist_Met <-
      as.matrix(vegan::vegdist(NMDS_Data_input[, 2:ncol(NMDS_Data_input)], method = method))
    NMDS <-
      vegan::metaMDS(Dist_Met, k = 2, trymax = 2000) # Bray curtis
    x_y_coord <-
      as.data.frame(vegan::scores(NMDS, display = "sites"))
