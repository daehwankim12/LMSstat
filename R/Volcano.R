...
Volcano <- function(data,
  ...
  for (i in 3:ncol(Part_dat_1)) {
    Volc_Dat[(i - 2), 1] <-
      log(mean(Part_dat_1[, i]) / mean(Part_dat_2[, i]), base = FC_log)
  }
  if (length(unique(data[["Data"]]$Group)) > 2) {
  ...
  Volc_Dat$Direction[Volc_Dat$log2FoldChange > FC_range[2] &
    Volc_Dat$pvalue < pval_intercept] <- "UP"
  Volc_Dat$Direction[Volc_Dat$log2FoldChange < FC_range[1] &
  ...
  sem_res <-
    ggplot2::ggplot(
      ...
      ggplot2::aes(
        x = log2FoldChange,
        y = -log(pvalue, base = pval_log),
        col = Direction
      )
    ) +
    ggplot2::geom_point(size = dotsize) +
    ggplot2::labs(
      title = paste0(G1, " divided by ", G2, " Volcano plot"),
      x = eval(parse(
        text = paste0("expression(Log[", FC_log, "]~(Fold~Change))")
      )),
      y = eval(parse(
        text = paste0("expression(Log[", pval_log, "]~(P-value))")
      ))
    ) +
    ...
    ggplot2::geom_vline(
      xintercept = FC_range[1],
      linetype = "dashed",
      col = c("grey")
    ) +
    ggplot2::geom_vline(
      xintercept = FC_range[2],
      linetype = "dashed",
      col = c("grey")
    ) +
    ggplot2::geom_hline(
      yintercept = -log(pval_intercept, base = pval_log),
      col = "black"
    )
  {
    if (fixed_limit == T) {
  ...
  {
    if (sig_label == T) {
      sem_res <- sem_res + ggrepel::geom_text_repel(
        size = 3,
        show.legend = F,
        ggplot2::aes(
          label = ifelse(
