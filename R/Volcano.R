...
Volcano <- function(data,
  ...
  data[["Data_renamed"]][, 1:(ncol(data[["Data_renamed"]]) - 1)] <- sapply(
  ...
  if (length(unique(data[["Data"]]$Group)) > 2) {
    if (asterisk == "Scheffe") {{ if (paste0(Comb[a, 1], "-", Comb[a, 2], "___", "ANO_posthoc") %in% colnames(p_val_data) == T) {
      ...
      part_p_val <- p_val_data[, paste0(Comb[a, 2], "-", Comb[a, 1], "___", "ANO_posthoc")]
      ...
      part_p_val <- p_val_data[, paste0(Comb[a, 2], " - ", Comb[a, 1], "___", "Kru_posthoc(Dunn)")]
      ...
      part_p_val <- p_val_data[, paste0(Comb[a, 2], "-", Comb[a, 1], "___", asterisk)]
    } }}
