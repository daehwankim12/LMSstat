...
SRH <- function(data,
  ...
  colnames(df) <- c("Multilevel", "Group", "Interaction")
  colnames(df_name) <- c("Ori", "LETTERS")
  ...
  for (a in 1:nrow(df_name)) {
    eval(parse(
      text = paste0(
        ...
        colnames(data)[a + 3],
        "~Multilevel*Group,data = data,verbose = F)$p.value[1:3]"
      )
    ))
  }
  print("Finished SRH")
  ...
  VennDiagram::venn.diagram(
    list(rownames(ML_sig), rownames(G_sig), rownames(Int_sig)),
    category.names = c("Multilevel", "Group", "Interaction"),
    filename = "Significant_variable_venndiagram.tiff",
    resolution = 1200,
    col = c("#440154ff", "#21908dff", "#fde725ff"),
    fill = c(
      ggplot2::alpha("#440154ff", 0.3),
      ggplot2::alpha("#21908dff", 0.3),
      ggplot2::alpha("#fde725ff", 0.3)
    ),
    cex = 4,
    lwd = 1,
    height = 16,
