#' Create and save boxplots for all metabolites with significance indicators
#'
#' This function automatically generates and saves boxplots for all metabolites in the dataset,
#' with asterisks indicating statistical significance between groups based on the selected
#' statistical test. Plots are saved as PNG files in a 'boxplot' directory.
#'
...
#' @param colour Vector of color codes used for different groups in the boxplots
...
#' @param width Width of the boxplots
#' @param size Line size for the boxplot outlines
...
#' @return Invisibly returns NULL, with plots saved to the 'boxplot' directory
#' @export
#'
#' @examples
#' \dontrun{
...
Boxplot_new <- function(data,
  ...
  draw <- function(i) {
    ...
    suppressMessages(
      ggplot2::ggsave(
        file.path("boxplot", sprintf("%s_box.png", sanitize(map[[safe[i]]]))),
        plot = p,
        device = ragg::agg_png,
        width = fig_width,
        height = fig_height,
